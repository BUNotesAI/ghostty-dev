...
- If code outside of `macos/` directory is modified, use
  `zig build -Demit-xcframework=true -Doptimize=ReleaseFast` before building
...
- Use `macos/build.nu` to build the macOS app, do not use `zig build`
  ...
  - Build: `macos/build.nu [--scheme "Ghostty Dev"] [--configuration Release] [--action build]`
  - Output: `macos/build/<configuration>/Ghostty Dev.app` (e.g. `macos/build/Release/Ghostty Dev.app`)
- Run unit tests directly with `macos/build.nu --configuration Debug --action test`
...
- Test AppleScript support:
  ...
  (2) Launch and activate the app via osascript using the absolute path
      ...
      `osascript -e 'tell application "<absolute path to build/Release/Ghostty Dev.app>" to activate'`
  ...
  (5) When done, quit via:
      `osascript -e 'tell application "<absolute path to build/Release/Ghostty Dev.app>" to quit'`
